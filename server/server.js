...
app.get('/hourly_charts_data', async (req, res) => {
    ...
    try {
        const response = await axios.get(`https://api.polygon.io/v2/aggs/ticker/${symbol}/range/1/hour/2024-03-27/${endDate}?adjusted=true&sort=asc&apiKey=${API_KEY_POLYGON}`);
        console.log('Fetching hourly data for', symbol, 'from', startDate, 'to', endDate);
        const data = response.data;
        if (!data || data.error) {
            return res.status(404).json({ error: 'No record has been found' });
        }
